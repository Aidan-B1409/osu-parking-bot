...
async def test_online_without_intents_and_clean_shutdown(cfg, monkeypatch):
    ...
    task = asyncio.create_task(presence.maintain_presence(cfg))
    try:
...
async def test_presence_retries_safely_and_rereads_token(cfg, monkeypatch, caplog, error):
    ...
    with pytest.raises(asyncio.CancelledError):
        await presence.maintain_presence(cfg)
...
async def test_missing_token_never_connects_and_wait_is_cancellable(cfg, monkeypatch, token, caplog):
...
async def test_scheduler_keeps_checking_while_presence_connects_and_closes_it(cfg, state, monkeypatch):
    ...
    async def maintain_presence(config):
        started.set()
        try:
